...
```{r grafico-uniforme, echo=FALSE}
a <- 2
b <- 6
curve(dunif(x, min = a, max = b), from = 0, to = 8, col = "blue", lwd = 2,
      ylab = "f(x)", main = "Distribución Uniforme U(2, 6)")
...
```{r grafico-normal, echo=FALSE}
mu <- 10
sigma <- 3

curve(dnorm(x, mean = mu, sd = sigma),
      from = mu - 4 * sigma, to = mu + 4 * sigma,
      col = "darkblue", lwd = 2, ylab = "f(x)", xlab = "x",
      main = "Distribución Normal N(10, 3)")
...
labels <- paste0("N(0, ", sds, ")")

x_vals <- seq(mu - 4 * max(sds), mu + 4 * max(sds), length.out = 1000)

plot(x_vals, dnorm(x_vals, mean = mu, sd = sds[1]), type = "l", lwd = 2,
     col = colors[1], ylab = "Densidad", xlab = "x",
     main = "Efecto de la varianza en la distribución normal")

for (i in 2:length(sds)) {
  lines(x_vals, dnorm(x_vals, mean = mu, sd = sds[i]), col = colors[i], lwd = 2)
}
...
mus <- c(-4, 0, 4)
colors <- c("red", "blue", "darkgreen")
labels <- paste0("N(", mus, ", 2)")

x_vals <- seq(min(mus) - 4 * sigma, max(mus) + 4 * sigma, length.out = 1000)

plot(x_vals, dnorm(x_vals, mean = mus[1], sd = sigma), type = "l", lwd = 2,
     col = colors[1], ylab = "Densidad", xlab = "x",
     main = "Efecto del cambio en la media")

for (i in 2:length(mus)) {
  lines(x_vals, dnorm(x_vals, mean = mus[i], sd = sigma), col = colors[i], lwd = 2)
}
...
plot(x_vals, y_vals, type = "l", lwd = 2, col = "darkblue",
     main = "Distribución Normal N(2000, 300)",
     ylab = "Densidad", xlab = "Salario")

# Sombra para P(X < 2300)
x_shade1 <- x_vals[x_vals <= 2300]
y_shade1 <- dnorm(x_shade1, mean = mu, sd = sigma)
polygon(c(x_shade1, rev(x_shade1)), c(rep(0, length(y_shade1)), rev(y_shade1)),
        col = rgb(0, 0, 1, 0.3), border = NA)

# Sombra para P(1850 < X < 2150)
x_shade2 <- x_vals[x_vals >= 1850 & x_vals <= 2150]
y_shade2 <- dnorm(x_shade2, mean = mu, sd = sigma)
polygon(c(x_shade2, rev(x_shade2)), c(rep(0, length(y_shade2)), rev(y_shade2)),
        col = rgb(1, 0.5, 0, 0.4), border = NA)

legend("topright",
       legend = c("P(X < 2300)", "P(1850 < X < 2150)"),
       fill = c(rgb(0, 0, 1, 0.3), rgb(1, 0.5, 0, 0.4)),
       border = NA, bty = "n")
```
...
```{r grafico-normal-estandar, echo=FALSE}
z_vals <- seq(-4, 4, length.out = 1000)
y_vals <- dnorm(z_vals)

plot(z_vals, y_vals, type = "l", lwd = 2, col = "darkblue",
     main = "Distribución Normal Estándar",
     ylab = "Densidad", xlab = "z")

# Sombra para P(Z < 1)
z_sombra <- z_vals[z_vals <= 1]
y_sombra <- dnorm(z_sombra)
polygon(c(z_sombra, rev(z_sombra)),
        c(rep(0, length(y_sombra)), rev(y_sombra)),
        col = rgb(0, 0, 1, 0.3), border = NA)

abline(v = 1, col = "blue", lty = 2)
legend("topright", legend = c("P(Z < 1)"),
       fill = rgb(0, 0, 1, 0.3), bty = "n")
```
...
plot(z_vals, y_vals, type = "l", lwd = 2, col = "black",
     ylab = "Densidad", xlab = "z",
     main = "P(Z > 1.25)")

z_right <- z_vals[z_vals > z]
y_right <- dnorm(z_right)
polygon(c(z_right, rev(z_right)),
        c(rep(0, length(y_right)), rev(y_right)),
...
z_left <- z_vals[z_vals <= z]
y_left <- dnorm(z_left)
polygon(c(z_left, rev(z_left)),
        c(rep(0, length(y_left)), rev(y_left)),
        col = rgb(0, 0, 1, 0.3), border = NA)
...
z_neg <- -z
z_left <- z_vals[z_vals <= z_neg]
y_left <- dnorm(z_left)
polygon(c(z_left, rev(z_left)),
        c(rep(0, length(y_left)), rev(y_left)),
        col = rgb(0, 1, 0, 0.3), border = NA)
abline(v = z_neg, lty = 2, col = "darkgreen")
```
...
z_middle <- z_vals[z_vals >= -z & z_vals <= z]
y_middle <- dnorm(z_middle)
polygon(c(z_middle, rev(z_middle)),
        c(rep(0, length(y_middle)), rev(y_middle)),
        col = rgb(0.5, 0, 0.5, 0.3), border = NA)
abline(v = c(-z, z), lty = 2, col = "purple")
```
...
z1 <- -1.85
prob1 <- 1 - pnorm(z1)
prob1  # Resultado ≈ 0.9678

# Crear los datos de la normal estándar
...
ggplot(df, aes(x, y)) +
  geom_line(size = 1.2) +
  geom_area(data = subset(df, x >= z1), fill = "lightgreen", alpha = 0.5) +
  geom_vline(xintercept = z1, linetype = "dashed") +
  annotate("text", x = 2.2, y = 0.05, label = "Área ≈ 0,9678", size = 5) +
  labs(
    title = expression(bold(P(Z >= -1.85))),
    x = "Z",
    y = "Densidad"
  ) +
  theme_minimal()
...
ggplot(df, aes(x, y)) +
  geom_line(size = 1.2) +
  geom_area(data = subset(df, x >= a & x <= b), fill = "lightblue", alpha = 0.5) +
  geom_vline(xintercept = c(a, b), linetype = "dashed") +
  annotate("text", x = 2.5, y = 0.05, label = "Área ≈ 0,3951", size = 5) +
  labs(
   title = expression(bold("P(0.20 < Z <= 1.95)")),
    x = "Z", y = "Densidad"
  ) +
  theme_minimal()
```
...
ggplot(df, aes(x, y)) +
  geom_line(size = 1.2) +
  geom_area(data = subset(df, x >= a2 & x <= b2), fill = "orange", alpha = 0.5) +
  geom_vline(xintercept = c(a2, b2), linetype = "dashed") +
  annotate("text", x = 1.5, y = 0.05, label = "Área ≈ 0,2716", size = 5) +
  labs(
    title = expression(bold("P(-1.60 <= Z <= -0.45)")),
    x = "Z", y = "Densidad"
  ) +
  theme_minimal()
```
...
```{r tabla-signo-abscisa, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
...
tabla <- data.frame(
  Suceso = c("$Z \\geq a$", "$Z \\leq a$"),
  `> 0,5` = c("Negativa", "Positiva"),
  `< 0,5` = c("Positiva", "Negativa"),
  `= 0,5` = c("0", "0")
)

kable(tabla, format = "html", escape = FALSE,
      col.names = c("Suceso", "> 0,5", "< 0,5", "= 0,5"),
      align = "c") %>%
...
```{r grafico-densidad-chi2, echo=FALSE}
# Valores de x
x <- seq(0, 20, length.out = 1000)

# Dibujamos varias curvas para distintos grados de libertad
plot(x, dchisq(x, df = 1), type = "l", lwd = 2, col = "red",
     ylim = c(0, 0.5),
     ylab = "Densidad", xlab = "x",
     main = expression("Funciones de densidad de" ~ chi[n]^2 ~ "para distintos grados de libertad"))

lines(x, dchisq(x, df = 3), col = "blue", lwd = 2)
lines(x, dchisq(x, df = 5), col = "green4", lwd = 2)
lines(x, dchisq(x, df = 10), col = "purple", lwd = 2)

legend("topright", legend = c("n = 1", "n = 5", "n = 10", "n = 50"),
       col = c("red", "blue", "green4", "purple"), lwd = 2, bty = "n")
```
...
```{r grafico-densidad-t, echo=FALSE}
x <- seq(-5, 5, length.out = 1000)

plot(x, dt(x, df = 1), type = "l", lwd = 2, col = "red",
     ylim = c(0, 0.4),
     ylab = "f(x), Densidad", xlab = "x",
     main = expression("Funciones de densidad de t"[n] ~ "para distintos grados de libertad"))

lines(x, dt(x, df = 3), col = "blue", lwd = 2)
lines(x, dt(x, df = 10), col = "green4", lwd = 2)
lines(x, dnorm(x), col = "black", lwd = 2, lty = 2)

legend("topright", legend = c("n = 1", "n = 3", "n = 10", "Normal(0,1)"),
       col = c("red", "blue", "green4", "black"), lwd = 2, lty = c(1, 1, 1, 2), bty = "n")
```
...
plot(x, df(x, df1 = 1, df2 = 5), type = "l", lwd = 2, col = "red",
     ylim = c(0, 1),
     ylab = "f(x), Densidad", xlab = "x",
     main = expression("Funciones de densidad de F"[n[1]*","*n[2]] ~ "para distintos grados de libertad"))

lines(x, df(x, df1 = 2, df2 = 10), col = "blue", lwd = 2)
lines(x, df(x, df1 = 5, df2 = 5), col = "green4", lwd = 2)
lines(x, df(x, df1 = 10, df2 = 20), col = "purple", lwd = 2)

legend("topright",
       legend = c("F(1, 5)", "F(2, 10)", "F(5, 5)", "F(10, 20)"),
       col = c("red", "blue", "green4", "purple"), lwd = 2, bty = "n")
```
...
tabla <- data.frame(
  Distribución = c("$$\\chi^2$$", 
                   "$$t_n$$", 
                   "$$F_{n_1,n_2}$$"),
  Aplicaciones = c(
    "• Pruebas de independencia (datos categóricos)<br>• Contrastes de bondad de ajuste<br>• Control de calidad<br>• Validación de modelos en economía y sociología",
    "• Estimación y contraste de medias con muestras pequeñas<br>• Análisis de impacto en encuestas piloto<br>• Evaluación de intervenciones económicas<br>• Detección de anomalías o fraude",
    "• Comparación de modelos en regresión<br>• Análisis de la varianza (ANOVA)<br>• Estudios regionales de desigualdad<br>• Contrastes globales de significatividad"
  )
)

kable(tabla, format = "html", escape = FALSE, align = "c",
      col.names = c("Distribución", "Aplicaciones en Ciencias Sociales")) %>%
...
```{r grafico-exponencial, echo=FALSE}
x <- seq(0, 10, length.out = 1000)
plot(x, dexp(x, rate = 0.5), type = "l", lwd = 2, col = "blue",
     ylab = "f(x)", xlab = "x",
     main = "Densidad de la distribución exponencial")

lines(x, dexp(x, rate = 1), col = "green4", lwd = 2)
lines(x, dexp(x, rate = 2), col = "red", lwd = 2)

legend("topright", legend = c(expression(lambda == 0.5),
                              expression(lambda == 1),
                              expression(lambda == 2)),
       col = c("blue", "green4", "red"), lwd = 2, bty = "n")
```
...
```{r grafico-densidad-gamma, echo=FALSE}
x <- seq(0, 20, length.out = 1000)
plot(x, dgamma(x, shape = 1, scale = 2), type = "l", lwd = 2, col = "red",
     ylim = c(0, 0.5), ylab = "f(x)", xlab = "x",
     main = expression("Funciones de densidad de la distribución Gamma"))

lines(x, dgamma(x, shape = 2, scale = 2), col = "blue", lwd = 2)
lines(x, dgamma(x, shape = 5, scale = 1), col = "green4", lwd = 2)

legend("topright", legend = c(expression(Gamma(1,2)), expression(Gamma(2,2)), expression(Gamma(5,1))),
       col = c("red", "blue", "green4"), lwd = 2, bty = "n")
```
...
```{r grafico-densidad-beta, echo=FALSE}
x <- seq(0, 1, length.out = 1000)
plot(x, dbeta(x, shape1 = 0.5, shape2 = 0.5), type = "l", lwd = 2, col = "red",
     ylim = c(0, 3), ylab = "f(x)", xlab = "x",
     main = "Densidad de la distribución Beta")

lines(x, dbeta(x, 2, 2), col = "blue", lwd = 2)
lines(x, dbeta(x, 2, 5), col = "green4", lwd = 2)
lines(x, dbeta(x, 5, 2), col = "purple", lwd = 2)

legend("topright",
       legend = c("Beta(0.5, 0.5)", "Beta(2, 2)", "Beta(2, 5)", "Beta(5, 2)"),
       col = c("red", "blue", "green4", "purple"), lwd = 2, bty = "n")
```
...
```{r, echo=FALSE}
# Valores para la densidad de Pareto: x desde 1 hasta 10, con xm = 1, α = 2.5
...
plot(x, f, type = "l", col = "blue", lwd = 2,
     ylab = "f(x)", xlab = "x",
     main = expression("Densidad de la distribución de Pareto (" ~ x[m] ~ " = 1, " ~ alpha == 2.5 ~ ")"))
...
tabla_continuas <- data.frame(
  Distribución = c("Normal", "Exponencial", "Chi-cuadrado", "t de Student", "F de Snedecor", "Gamma", "Beta", "Pareto"),
  Variable = c("Variable continua simétrica", 
               "Tiempo entre eventos", 
               "Suma de cuadrados normales estándar", 
               "Media muestral con varianza desconocida", 
               "Cociente de dos chi-cuadrado normalizados", 
               "Variable con crecimiento exponencial", 
               "Proporciones entre 0 y 1", 
               "Distribución de ingresos/riqueza"),
  Parámetros = c("\\(\\mu,\\ \\sigma\\)", 
                 "\\(\\lambda\\)", 
                 "\\(k\\)", 
                 "\\(n\\)", 
                 "\\(n_1,\\ n_2\\)", 
  ...
  `Densidad \\(f(x)\\)` = c(
    ...
    "\\(\\frac{\\Gamma(\\frac{n+1}{2})}{\\sqrt{n\\pi}\\Gamma(n/2)} \\left(1 + \\frac{x^2}{n}\\right)^{-\\frac{n+1}{2}}\\)",
    "\\(\\frac{\\Gamma((n_1 + n_2)/2)}{\\Gamma(n_1/2)\\Gamma(n_2/2)} \\cdot \\frac{(n_1/n_2)^{n_1/2} x^{n_1/2 - 1}}{(1 + \\frac{n_1}{n_2} x)^{(n_1+n_2)/2}}\\)",
  ...
  Media = c("\\(\\mu\\)", 
            ...
            "\\(k\\)", 
            "\\(0\\) si \\(n \\gt 1\\)", 
            "\\(\\frac{n_2}{n_2 - 2}\\) si \\(n_2 \\gt 2\\)", 
            "\\(\\alpha\\beta\\)", 
            "\\(\\frac{\\alpha}{\\alpha + \\beta}\\)", 
            "\\(\\frac{\\alpha x_m}{\\alpha - 1}\\) si \\(\\alpha \\gt 1\\)"),
  Varianza = c("\\(\\sigma^2\\)", 
               "\\(\\frac{1}{\\lambda^2}\\)", 
               "\\(2k\\)", 
               "\\(\\frac{n}{n - 2}\\) si \\(n \\gt 2\\)", 
               "\\(\\frac{2n_2^2(n_1 + n_2 - 2)}{n_1(n_2 - 2)^2(n_2 - 4)}\\) si \\(n_2 \\gt 4\\)", 
               "\\(\\alpha\\beta^2\\)", 
               "\\(\\frac{\\alpha\\beta}{(\\alpha + \\beta)^2(\\alpha + \\beta + 1)}\\)", 
               "\\(\\frac{\\alpha x_m^2}{(\\alpha - 1)^2(\\alpha - 2)}\\) si \\(\\alpha \\gt 2\\)")
)

kable(tabla_continuas, escape = FALSE, booktabs = TRUE) %>%
