...
# Probabilidades arbitrarias que suman 1 (genéricas)
f <- c(0.1, 0.2, 0.3, 0.15, 0.15, 0.1)

# Función de distribución acumulada
F <- cumsum(f)

# Función de cuantía: gráfico de "palos"
plot(x, f, type = "h", lwd = 2, col = "blue",
     main = "Función de cuantía f(x)",
     xlab = "Valores de X", ylab = "P(X = x)")
points(x, f, pch = 16, col = "blue")

# Función de distribución acumulada: gráfico escalonado
...
```{r grafico-fda-continua, echo=FALSE, fig.width=7, fig.height=4, warning=FALSE}
# Valores del eje x
x <- seq(-4, 4, length.out = 300)
...
# Dibujar densidad
plot(x, f, type = "l", lwd = 2, col = "blue",
     main = "Área bajo la curva entre a y b",
     xlab = "Valores de X", ylab = "f(x)")
abline(h = 0, col = "gray")

# Rellenar el área entre a y b
x_fill <- seq(a, b, length.out = 200)
y_fill <- dnorm(x_fill)
polygon(c(a, x_fill, b), c(0, y_fill, 0),
        col = rgb(0.2, 0.6, 1, 0.4), border = NA)

# Marcar a y b con líneas verticales
segments(a, 0, a, dnorm(a), lty = 2)
segments(b, 0, b, dnorm(b), lty = 2)

# Dibujar los puntos (a, f(a)) y (b, f(b))
points(a, dnorm(a), pch = 19, col = "red")
points(b, dnorm(b), pch = 19, col = "red")

# Etiquetas con coordenadas
text(a, dnorm(a), paste0("a = ", a), pos = 4)
text(b, dnorm(b), paste0("b = ", b), pos = 4)
...
# Densidad marginal de X
curve(2*x, from=0, to=1,
      xlab="x", ylab="f_X(x)",
      main="Densidad marginal de X", lwd=2, col="darkgreen")
```
...
curve(2*x, from=0, to=1,
      xlab="y", ylab="f_Y(y)",
      main="Densidad marginal de Y", lwd=2, col="purple")
```

```{r echo=FALSE}
# Malla para la densidad conjunta
x <- seq(0, 1, length = 30)
y <- seq(0, 1, length = 30)
z <- outer(x, y, function(x,y) 4*x*y)

persp(x, y, z,
      theta = 30, phi = 30, expand = 0.5,
      col = "lightblue",
      xlab = "x", ylab = "y", zlab = "f(x,y)",
      main = "Densidad conjunta f(x,y)")
```
